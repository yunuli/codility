/**
 ...
 Bob computed an array B and then mistakenly deleted A. He now intends to find every valid array A from which his program would produce B. Count the number of such arrays A. Since the answer could be very big, return it modulo 10e9+7.

 Write a function:

 class Solution { public int solution(int[] B, int M); }
 that, given an integer M and a zero-indexed array B with N integers, returns the remainder from the division by 10e9+7 of the number of valid arrays A from which Bob would get B. You can assume that there is at least one such array.
 ...
 For the following data: M = 105, B = [0, 0] there are 5000050000 possible arrays (the first element in array A can be anything in the range 1..10e5 and the second element can be either equal or smaller), so the function should return 49965 (as we are taking modulo 10e9+7).
